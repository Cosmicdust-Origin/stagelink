...
export default async function GroupsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {canEdit ? (
        <CollapsibleSection label="그룹 추가">
          <GroupCreateForm />
        </CollapsibleSection>
      ) : null}
      {groups?.length ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {groups.map((group) => (
            <Link
              ...
              className="rounded-lg border border-white/10 bg-white/[0.04] p-4 hover:bg-white/[0.07]"
            >
        ...
        <EmptyState
          ...
          description={
            canEdit
              ? "위 입력 영역에서 첫 그룹을 추가하세요."
              : "관리자 계정으로 그룹을 추가할 수 있습니다."
          }
        />
      )}
    </div>
